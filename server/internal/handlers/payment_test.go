...
func setupPaymentHandlerTest(t *testing.T, setupRepos func(*testutil.MockTaskRepository, *testutil.MockTransactionRepository, *testutil.MockWalletRepository)) (*PaymentHandler, func()) {
	...
	// Create services
	walletService := services.NewWalletService(walletRepo, walletTxRepo, mockDB)
	paymentService := services.NewPaymentService(txRepo, taskRepo, walletService, nil)

	handler := NewPaymentHandler(nil, paymentService, "http://localhost:3000")
