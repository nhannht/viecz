...
import { VieczTransactionRowComponent } from '../shared/components/viecz-transaction-row.component';
import { VieczBankSelectComponent } from '../shared/components/viecz-bank-select.component';
...
@Component({
  ...
  imports: [
    ...
    VieczTransactionRowComponent,
    VieczBankSelectComponent,
  ],
  template: `
        ...
        <div class="flex flex-col gap-4">
          ...
          <viecz-card>
            ...
            <div class="flex gap-3 items-end">
              <viecz-input [label]="t('wallet.amountLabel')" type="number"
                [step]="1000" [min]="2000"
                [(ngModel)]="depositAmount" name="depositAmount"
                [error]="depositError" />
              @if (depositing()) {
                <viecz-spinner size="sm" [label]="t('wallet.depositing')" />
              } @else {
                <viecz-button variant="primary" [label]="t('wallet.depositButton')"
                  (clicked)="deposit()" />
              }
            </div>
          ...
          <viecz-card>
            ...
            <div class="flex gap-3 items-end flex-wrap">
              ...
              <viecz-input [label]="t('wallet.amountLabel')" type="number"
                ...
                [error]="withdrawError" />
              @if (withdrawing()) {
                <viecz-spinner size="sm" [label]="t('wallet.withdrawing')" />
              } @else {
                <viecz-button variant="primary" [label]="t('wallet.withdrawButton')"
                  (clicked)="withdraw()" />
              }
            </div>
            ...
            <div class="mt-3">
              <viecz-button variant="secondary" [label]="showBankAccountForm() ? t('common.close') : t('wallet.manageBankAccounts')"
                ...
                @if (bankAccounts().length === 0) {
                  ...
                  @for (ba of bankAccounts(); track ba.id) {
                    <div class="flex items-center justify-between py-2 border-b border-border last:border-b-0">
                      ...
                      <viecz-button variant="secondary" [label]="t('common.delete')"
                ...
                <div class="mt-4">
                  ...
                  <div class="flex flex-col gap-3">
                    <viecz-bank-select
                      ...
                      [(ngModel)]="newBankBin"
                      name="newBankBin" />
                    <viecz-input [label]="t('wallet.accountNumber')" type="text"
                      [(ngModel)]="newBankAccountNumber" name="newBankAccountNumber" />
                    <viecz-input [label]="t('wallet.accountHolderName')" type="text"
                      [(ngModel)]="newBankAccountHolderName" name="newBankAccountHolderName" />
                    @if (addingBankAccount()) {
                      <viecz-spinner size="sm" />
                    } @else {
                      <viecz-button variant="primary" [label]="t('wallet.addBankAccount')"
            ...
            @if (transactions().length === 0) {
              ...
              @if (hasMore()) {
                <div class="text-center pt-3">
                  <viecz-button variant="secondary" [label]="t('common.loadMore')"
                    (clicked)="loadMore()" />
                </div>
              }
            }
