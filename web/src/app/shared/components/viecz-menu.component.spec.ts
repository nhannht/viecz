...
@Component({
  standalone: true,
  imports: [VieczMenuComponent],
  template: `
    <div class="wrapper">
      <viecz-menu [open]="open()" (closed)="closedCount = closedCount + 1">
        <button class="viecz-menu-item">Item 1</button>
        <button class="viecz-menu-item">Item 2</button>
      </viecz-menu>
    </div>
  `,
})
...
describe('VieczMenuComponent', () => {
  ...
  it('should NOT emit closed on click inside menu', () => {
    host.open.set(true);
    fixture.detectChanges();
    const menu = fixture.nativeElement.querySelector('viecz-menu');
    menu.click();
    expect(host.closedCount).toBe(0);
  });
