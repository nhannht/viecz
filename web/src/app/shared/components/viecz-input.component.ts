 ...
 * @example
 * ```html
 * <viecz-input label="Email" placeholder="you@example.com" type="email"
 *   [formControl]="emailCtrl" [error]="emailCtrl.hasError('required') ? 'Required' : ''" />
 * ```
 */
@Component({
  selector: 'viecz-input',
  standalone: true,
  template: `
  ...
  `,
  providers: [
    { provide: NG_VALUE_ACCESSOR, useExisting: forwardRef(() => VieczInputComponent), multi: true },
  ],
})
export class VieczInputComponent implements ControlValueAccessor {
