...
@Component({
  standalone: true,
  imports: [VieczChatBubbleComponent],
  template: `<viecz-chat-bubble [message]="msg()" [isMine]="mine()" />`,
})
...
describe('VieczChatBubbleComponent', () => {
  ...
  describe('template conditional toggles', () => {
    ...
    it('should toggle read receipt from hidden to shown (is_read: false -> true with isMine)', () => {
      ...
      host.msg.set({ ...mockMessage, is_read: false });
      fixture.detectChanges();
      expect(fixture.nativeElement.querySelector('viecz-icon')).toBeNull();

      // is_read=true: icon appears
      host.msg.set({ ...mockMessage, is_read: true });
      fixture.detectChanges();
      expect(fixture.nativeElement.querySelector('viecz-icon')).toBeTruthy();
    });
