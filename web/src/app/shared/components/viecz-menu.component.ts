 ...
 * `<button>` or `<a>` with the `viecz-menu-item` class.
 *
 * @example
 * ```html
 * <viecz-menu [open]="menuOpen()" (closed)="menuOpen.set(false)">
 *   <button class="viecz-menu-item" (click)="onProfile()">Profile</button>
 *   <button class="viecz-menu-item" (click)="onLogout()">Logout</button>
 * </viecz-menu>
 * ```
 */
@Component({
  ...
  styles: `
    ...
    ::ng-deep .viecz-menu-item:hover {
      background-color: color-mix(in srgb, var(--color-fg) 5%, transparent);
    }
    ::ng-deep .viecz-menu-item + .viecz-menu-item {
      border-top: 1px solid color-mix(in srgb, var(--color-border) 40%, transparent);
    }
  `,
})
export class VieczMenuComponent {
