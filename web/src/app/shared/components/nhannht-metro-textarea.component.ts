 ...
 * Shares the same visual language as `NhannhtMetroInputComponent`.
 *
 * Replaces `MatFormField` + `<textarea matInput>` from Angular Material.
 *
 * @example
 * ```html
 * <nhannht-metro-textarea label="Description" placeholder="Describe the task..."
 *   [rows]="6" [formControl]="descCtrl" />
 * ```
 */
@Component({
  selector: 'nhannht-metro-textarea',
  standalone: true,
  template: `
  ...
  `,
  providers: [
    { provide: NG_VALUE_ACCESSOR, useExisting: forwardRef(() => NhannhtMetroTextareaComponent), multi: true },
  ],
})
export class NhannhtMetroTextareaComponent implements ControlValueAccessor {
