...
import { VieczBadgeComponent } from '../shared/components/viecz-badge.component';
import { VieczApplicationCardComponent } from '../shared/components/viecz-application-card.component';
...
@Component({
  ...
  template: `
    ...
    @if (loading()) {
        ...
        <div>
          <viecz-card>
              ...
              @if (isRequester() && task()!.status === 'open') {
                <span class="ml-auto flex gap-1">
                  <button class="text-muted hover:text-fg transition-colors duration-200"
                    ...
                    <viecz-icon name="edit" [size]="20" />
                  </button>
                  <button class="text-muted hover:text-fg transition-colors duration-200"
                          (click)="confirmDelete()"
                          [attr.aria-label]="t('task.deleteTask')">
                    <viecz-icon name="delete" [size]="20" />
                  </button>
                </span>
              }
            </div>
            ...
            <div class="flex justify-between items-center mb-4">
              <viecz-badge [label]="t(statusKey(task()!.status))" [status]="badgeStatus()" />
            ...
            <div class="flex flex-col gap-2 mt-4">
              <div class="flex items-center gap-2 text-[13px] text-muted">
                <viecz-icon name="location_on" [size]="18" />
              ...
              @if (task()!.deadline) {
                <div class="flex items-center gap-2 text-[13px] text-muted">
                  <viecz-icon name="schedule" [size]="18" />
                  <span>{{ t('task.deadline') }} {{ task()!.deadline | timeAgo }}</span>
                  @if (task()!.is_overdue) {
                    <viecz-badge [label]="t('task.overdue')" status="cancelled" />
                  }
                </div>
              }
              <div class="flex items-center gap-2 text-[13px] text-muted">
                <viecz-icon name="calendar_today" [size]="18" />
              ...
              @if (!auth.isAuthenticated()) {
                <div class="border-2 border-fg p-6 w-full">
                  ...
                  <div class="flex gap-4 items-center">
                    <a routerLink="/login">
                      <viecz-button variant="primary" [label]="t('task.registerToApply')" />
                    </a>
                ...
                @if (task()!.status === 'in_progress') {
                  <viecz-button variant="primary" [label]="t('task.markComplete')" (clicked)="completeTask()" />
                }
              } @else if (task()!.status === 'open' && !task()!.user_has_applied && !task()!.is_overdue) {
                <viecz-button variant="primary" [label]="t('task.apply')" (clicked)="navigateToApply()" />
              } @else if (task()!.user_has_applied) {
                <span class="flex items-center gap-1 text-[13px] text-muted font-bold">
                  <viecz-icon name="check" [size]="18" /> {{ t('task.applied') }}
                </span>
              }
            </div>
          </viecz-card>
      ...
      <viecz-dialog [open]="showDeleteDialog()" [title]="t('task.cancelDialogTitle')"
      ...
      <viecz-dialog [open]="showEscrowDialog()" [title]="t('task.escrowDialogTitle')"
