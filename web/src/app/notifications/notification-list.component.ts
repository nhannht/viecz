          ...
          <div class="flex items-center justify-between px-6 py-4 border-b border-border">
            ...
            @if (notifications().length > 0) {
              <button class="flex items-center gap-1 bg-transparent border-none cursor-pointer
                      ...
                      (click)="markAllRead()">
                <viecz-icon name="done_all" [size]="16" /> {{ t('notifications.markAllRead') }}
              </button>
            }
          </div>
          ...
          @if (loading()) {
            ...
            @for (n of notifications(); track n.id) {
              <div class="notif-item flex items-start gap-3 px-6 py-3 cursor-pointer
                ...
                <viecz-icon [name]="getTypeIcon(n.type)" [size]="20" />
            ...
            @if (hasMore()) {
              <div class="px-6 py-3">
                <viecz-button variant="secondary" [label]="t('common.loadMore')" [fullWidth]="true" (clicked)="loadMore()" />
              </div>
            }
          }
        </div>
